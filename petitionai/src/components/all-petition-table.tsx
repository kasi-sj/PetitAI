...
const AllPetitionTable = ({ isAdmin }: { isAdmin?: true }) => {
  ...
  const { isPending, error, isFetching, data, refetch } = useQuery({
    queryKey: ["org-user-petitions-all", userId, adminUser?.userId, pageNumber],
    queryFn: () => {
      if (isAdmin) {
      ...
      return getOrganizationUserPetitions(userId || "", false, pageNumber);
    },
  ...
  return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold">All Petitions</h2>
